...
export default function MasterDataTable() {
  ...
  const dataSelectSearch =
    querySelectParams.statusTypeFeeGroup.length === 0
      ? {
          statusTypeFeeGroup: [
  ...
  const deleteMutation = useMutation({
    mutationFn: () => deleteTypeFeeGroup(selectedRowKeys),
    onSuccess: (data) => {
      if (data.status) {
  ...
  const handleSearchSelect = (
    ...
    const newQueryParams = {
      ...
      searchAll: '',
      statusTypeFeeGroup:
        filters.statusTypeFeeGroup?.length !== 0 && filters.statusTypeFeeGroup
          ? (filters.statusTypeFeeGroup as string[])
          : [],
    };
  ...
  const columns: ProColumns<TypeFeeGroupTable>[] = [
    ...
    {
      title: (
        <div className={style.title}>{translateTypeFeeGroup('status')}</div>
      ),
      width: 120,
      dataIndex: 'statusTypeFeeGroup',
      key: 'statusTypeFeeGroup',
      align: 'center',
      ...
      filterSearch: false,
      filteredValue: querySelectParams.statusTypeFeeGroup || null,
      filterIcon: () => {
        return (
          <FilterFilled
            style={{
    ...
    },
    {
      ...
      render: (value) => (
        <div style={{ display: 'flex' }}>
          <Button
            onClick={() =>
              router.push(ROUTERS.TYPE_FEE_GROUP_EDIT(value as string, true))
            }
            icon={<EyeOutlined />}
            style={{
              marginRight: '10px',
  ...
  const handleEditCustomer = (id: string) => {
    router.push(ROUTERS.TYPE_FEE_GROUP_EDIT(id));
  };
  ...
  const handleOnDoubleClick = (
    ...
    if (!target.closest('button')) {
      router.push(ROUTERS.TYPE_FEE_GROUP_EDIT(record.key, true));
    }
  };

  const handleCreate = () => {
    router.push(ROUTERS.TYPE_FEE_GROUP_CREATE);
  };
  ...
  const exportData = useMutation({
    mutationFn: () =>
      exportTableFile({
        ids: selectedRowKeys,
        status: querySelectParams.statusTypeFeeGroup,
      }),
    onSuccess: (data) => {
      ...
      {locationsQuerySearch.isLoading ? (
        ...
        <>
          ...
          <Table
            dataTable={dataTable}
            columns={columns}
            headerTitle={translateTypeFeeGroup('title')}
