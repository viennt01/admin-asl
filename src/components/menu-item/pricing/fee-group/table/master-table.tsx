...
export default function MasterDataTable() {
  ...
  const deleteMutation = useMutation({
    mutationFn: () => deleteFeeGroup(selectedRowKeys),
    onSuccess: (data) => {
      if (data.status) {
        successToast(data.message);
        queryClient.invalidateQueries({
          queryKey: [TYPE_TABS.GET_OTHER_CHARGES_PRICING_BY_MASTER_DATA],
        });
        setSelectedRowKeys([]);
      } else {
  ...
  const handleSearchSelect = (
    ...
    const newQueryParams = {
      ...
      searchAll: '',
      statusFeeGroup:
        filters.statusFeeGroup?.length !== 0 && filters.statusFeeGroup
          ? (filters.statusFeeGroup as string[])
          : [],
    };
  ...
  const columns: ProColumns<FeeGroupTable>[] = [
    ...
    //       {translateFeeGroup('type_of_fee_group')}
    //     </div>
    //   ),
    //   width: 150,
    //   dataIndex: 'cityID',
    //   key: 'cityID',
    //   align: 'left',
    //   filteredValue: [querySelectParams.typeFeeGroupID] || null,
    //   filters:
    ...
    //         style={{
    //           color:
    //             querySelectParams.typeFeeGroupID?.length !== 0
    ...
    //   render: (_, value) => {
    //     return <div>{value.typeFeeGroupName}</div>;
    //   },
    // },
    {
  ...
  const handleOnDoubleClick = (
    ...
    if (!target.closest('button')) {
      router.push(ROUTERS.PRICING_FEE_GROUP_EDIT(record.key, true));
    }
  };

  const handleCreate = () => {
    router.push(ROUTERS.PRICING_FEE_GROUP_CREATE);
  };
  ...
  const exportData = useMutation({
    ...
    onSuccess: (data) => {
      ...
      link.href = url;
      link.setAttribute('download', `ASL_FEE_GROUP${getSystemDate()}.xlsx`);
  ...
  const importData = useMutation({
    ...
    onSuccess: (data) => {
      ...
      window.URL.revokeObjectURL(url);
      queryClient.invalidateQueries({
        queryKey: [TYPE_TABS.GET_OTHER_CHARGES_PRICING_BY_REQUEST_DATA],
      });
