...
import { ICustomPricingFCLDetailDTOs, IFormValues } from '../interface';
...
const FCL = ({ form, isCheckPermissionEdit, optionUnit }: Props) => {
  ...
  useEffect(() => {
    ...
    if (
      form.getFieldValue('customPricingFCLDetailDTOs') &&
      countLoadData === 0
    ) {
      setDataRequire(
        form
          .getFieldValue('customPricingFCLDetailDTOs')
          .map((item: ICustomPricingFCLDetailDTOs) => {
            return {
              key: item.customPricingFCLDetailID || '',
              unitID: item.unitID || '',
  ...
  // setFieldValue customPricingFCLDetailDTOs
  useEffect(() => {
    if (countLoadData === 1 && dataSource.length !== 0) {
      form.setFieldValue(
        ...
        dataSource.map((item) => {
          return {
            customPricingFCLDetailID: item.key,
            unitID: item.unitID,
            basePriceRedLane: item.basePriceRedLane,
            basePriceGreenLane: item.basePriceGreenLane,
            basePriceYellowLane: item.basePriceYellowLane,
